...
class TreeTooltip:
    ...
    def __init__(self, tree, get_text_func):
        self.tree = tree
    ...
    def _on_motion(self, event):
        """Show tooltip for item under cursor"""
        ...
        if item == self.last_item and self.tooltip:
            return  # Same item, don't recreate
        ...
        # Get item position for tooltip placement
        x = event.x_root + 15
        y = event.y_root + 10
...
class FileBrowser(ttk.Frame):
    """Embedded file browser with hierarchical tree view"""

    def __init__(self, parent, initial_dir, on_file_select, **kwargs):
        super().__init__(parent, **kwargs)
        self.on_file_select = on_file_select
        self.initial_dir = initial_dir
        self.node_paths = {}  # Maps tree node IDs to full paths

        self._create_widgets()
        self._populate_root(initial_dir)

        # Add tooltip to show full path on hover
        self.tooltip = TreeTooltip(self.tree, self._get_tooltip_text)

    def _get_tooltip_text(self, item):
        """Get tooltip text (full path) for tree item"""
        if item in self.node_paths:
            return self.node_paths[item]
        return None
    ...
    def _create_widgets(self):
        ...
        self.path_entry = ttk.Entry(path_frame, textvariable=self.path_var, font=MONO_FONT)
        self.path_entry.pack(side="left", fill="x", expand=True)
        self.path_entry.bind('<Return>', self._on_path_enter)

        ttk.Button(path_frame, text="Go", width=4, command=self._on_path_enter).pack(side="right", padx=2)

        # Treeview for files/folders
        ...
        self.tree.column("size", width=70, stretch=False, anchor="e")
        ...
        # Bindings
        self.tree.bind('<<TreeviewOpen>>', self._on_open)  # When folder is expanded via arrow
        self.tree.bind('<Button-1>', self._on_click)  # Single click to open folder
        self.tree.bind('<Double-1>', self._on_double_click)  # Double-click on audio file

        # Configure tags for styling
        self.tree.tag_configure("folder", foreground="#0066cc")
        self.tree.tag_configure("audio", foreground="#006600")
        self.tree.tag_configure("video", foreground="#990099")
        self.tree.tag_configure("other", foreground="#888888")
    ...
    def _populate_root(self, path):
        """Populate tree starting from root path"""
        ...
        root_id = self.tree.insert("", "end", text=f"📁 {root_name}", values=("",),
                                    tags=("folder",), open=True)
        self.node_paths[root_id] = path

        # Populate its contents
        self._populate_folder(root_id, path)

    def _populate_folder(self, parent_id, path):
        """Populate a folder's contents as children"""
        # Get current children
        children = self.tree.get_children(parent_id)

        # Check if already populated (has real children, not just dummy)
        if children:
            try:
                first_text = self.tree.item(children[0], "text")
                if first_text != "_dummy_":
                    return  # Already populated
                # Remove dummy
                self.tree.delete(children[0])
            except:
                pass  # Item already deleted, continue

        try:
            entries = os.listdir(path)
        except PermissionError:
            self.tree.insert(parent_id, "end", text="[Permission Denied]", values=("",))
            return
        except Exception as e:
            self.tree.insert(parent_id, "end", text=f"[Error]", values=("",))
            return
        ...
        for name, full_path in folders:
            folder_id = self.tree.insert(parent_id, "end", text=f"📁 {name}",
                                          values=("",), tags=("folder",))
            self.node_paths[folder_id] = full_path
            # Add dummy child so folder shows as expandable
            self.tree.insert(folder_id, "end", text="_dummy_")

        # Add audio and video files
        for name, full_path, size in files:
            if self._is_audio_file(name):
                file_id = self.tree.insert(parent_id, "end", text=f"🎵 {name}",
                                            values=(self._format_size(size),), tags=("audio",))
                self.node_paths[file_id] = full_path
            elif self._is_video_file(name):
                file_id = self.tree.insert(parent_id, "end", text=f"🎬 {name}",
                                            values=(self._format_size(size),), tags=("video",))
                self.node_paths[file_id] = full_path

    def _on_open(self, event):
        """Handle folder expansion via arrow"""
        item = self.tree.focus()
        if item and item in self.node_paths:
            path = self.node_paths[item]
            if os.path.isdir(path):
                self._populate_folder(item, path)

    def _on_click(self, event):
        """Handle single click - open folder"""
        # Get item under cursor
        item = self.tree.identify_row(event.y)
        print(f"DEBUG: Click on item={item}, y={event.y}")
        if not item:
            return
        ...
        if "folder" in tags and item in self.node_paths:
            # Toggle folder open/closed
            is_open = self.tree.item(item, "open")
            print(f"DEBUG: Folder is_open={is_open}")
            if is_open:
                self.tree.item(item, open=False)
            else:
                self.tree.item(item, open=True)
                path = self.node_paths[item]
                print(f"DEBUG: Populating {path}")
                self._populate_folder(item, path)

    def _on_double_click(self, event):
        """Handle double-click - add audio/video file to list"""
        item = self.tree.focus()
        if not item:
            return
        ...
        if ("audio" in tags or "video" in tags) and item in self.node_paths:
            full_path = self.node_paths[item]
            if self.on_file_select:
                self.on_file_select(full_path)

    def _on_path_enter(self, event=None):
        """Handle Enter key or Go button in path entry"""
        path = self.path_var.get()
        if os.path.isdir(path):
            self._populate_root(path)

    def add_selected_files(self):
        """Add all selected audio/video files to the list"""
